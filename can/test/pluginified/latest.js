...
define('can/component/component_test', [
    ...
    test('basic tabs', function () {
        ...
        can.append(can.$('#qunit-fixture'), template({ foodTypes: foodTypes }));
        var testArea = can.$('#qunit-fixture')[0], lis = testArea.getElementsByTagName('li');
    ...
    test('lexical scoping', function () {
        ...
        equal(can.trim(hello[0].innerHTML), 'Hello', 'If no template is provided to can.Component, treat <content> bindings as dynamic.');
        can.remove(can.$('#qunit-fixture > *'));
    });
    ...
    test('treecombo', function () {
        ...
        setTimeout(function () {
            ...
            var treecombo = can.$('#qunit-fixture treecombo')[0], breadcrumb = can.$('#qunit-fixture .breadcrumb')[0], breadcrumbLIs = breadcrumb.getElementsByTagName('li'), options = can.$('#qunit-fixture .options')[0], optionsLis = options.getElementsByTagName('li');
    ...
    test('deferred grid', function () {
        ...
        var template = can.view.mustache('<grid deferreddata=\'viewModel.deferredData\'>' + '{{#each items}}' + '<tr>' + '<td width=\'40%\'>{{first}}</td>' + '<td width=\'70%\'>{{last}}</td>' + '</tr>' + '{{/each}}' + '</grid>');
        can.append(can.$('#qunit-fixture'), template({ viewModel: viewModel }));
    ...
    test('nextprev', function () {
        ...
        var prev = can.$('#qunit-fixture .prev')[0], next = can.$('#qunit-fixture .next')[0];
    ...
    test('page-count', function () {
        ...
        can.append(can.$('#qunit-fixture'), template(new can.Map({ paginator: paginator })));
    ...
    test('self closing content tags', function () {
        ...
        can.append(can.$('#qunit-fixture'), template({ site: 'CanJS' }));
        equal(can.$('#qunit-fixture span').length, 1, 'there is an h1');
    });
    test('can.viewModel utility', function () {
        ...
        can.append(can.$('#qunit-fixture'), can.view.mustache('<my-taggy-tag id=\'x\'></my-taggy-tag>')());
    ...
    test('setting passed variables - two way binding', function () {
        ...
        can.append(can.$('#qunit-fixture'), template({}));
        var testArea = can.$('#qunit-fixture')[0], buttons = testArea.getElementsByTagName('button');
    ...
    test('helpers reference the correct instance (#515)', function () {
        ...
        can.append(can.$('#qunit-fixture'), template({}));
        var testArea = can.$('#qunit-fixture')[0], myTexts = testArea.getElementsByTagName('my-text');
    ...
    test('access hypenated attributes via camelCase or hypenated', function () {
        ...
        can.append(can.$('#qunit-fixture'), template({}));
        var testArea = can.$('#qunit-fixture')[0], hyphen = testArea.getElementsByTagName('hyphen');
    ...
    test('Scope as Map constructors should follow \'@\' default values (#657)', function () {
        ...
        can.append(can.$('#qunit-fixture'), frag);
        equal(can.viewModel(can.$('panel')[0]).attr('title'), 'Libraries');
    });
...
define('can/construct/construct_test', [
...
define('can/compute/compute_test', [
    ...
    test('can.Compute.read can read a promise (#179)', function () {
        ...
        var map = new can.Map();
        var c = can.compute(function () {
                return can.Compute.read({ map: map }, [
                    'map',
                    'data',
                    'value'
                ]).value;
...
define('can/view/view_test', [
    ...
    test('live lists are rendered properly when batch updated (#680)', function () {
        var div1 = document.createElement('div'), div2 = document.createElement('div'), template = '{{#if items.length}}<ul>{{#each items}}<li>{{.}}</li>{{/each}}</ul>{{/if}}', stacheTempl = can.stache(template), mustacheTempl = can.mustache(template);
...
define('can/view/ejs/ejs_test', [
    ...
    test('A non-escaping live magic tag within a control structure and no leaks', function () {
        ...
        var text = '<div><% items.each(function(ob) { %>' + '<%== ob.attr(\'html\') %>' + '<% }); %></div>', items = new can.List([{ html: '<label>Hello World</label>' }]), compiled = new can.EJS({ text: text }).render({ items: items }), div = can.$('#qunit-fixture')[0];
        div.innerHTML = '';
        can.append(can.$('#qunit-fixture'), can.view.frag(compiled));
...
/*can@2.2.1#view/mustache/spec/specs/spec*/
define('can/view/mustache/spec/specs/spec', [], function () {
    return window.MUSTACHE_SPECS = [];
});
/*can@2.2.1#view/mustache/spec/specs/comments*/
...
/*can@2.2.1#view/mustache/spec/specs/interpolation*/
...
/*can@2.2.1#view/mustache/spec/specs/inverted*/
...
/*can@2.2.1#view/mustache/spec/specs/partials*/
...
/*can@2.2.1#view/mustache/spec/specs/sections*/
...
/*can@2.2.1#view/mustache/spec/specs/specs*/
...
define('can/view/stache/stache_test', [
    ...
    test('multiple tbodies in table hookup', function () {
        ...
        equal(can.$('#qunit-fixture table tbody').length, 2, 'two tbodies');
    });
    test('Observe with array attributes', function () {
...
define('can/control/control_test', [
    'can/control/control',
    'steal-qunit'
], function () {
...
define('can/view/mustache/mustache_test', [
    ...
    try {
        if (can.$('<col>').length) {
            test('<col> inside <table> renders correctly (#1013)', 1, function () {
                var template = '<table><colgroup>{{#columns}}<col class="{{class}}" />{{/columns}}</colgroup><tbody></tbody></table>';
                var frag = can.mustache(template)({ columns: new can.List([{ 'class': 'test' }]) });
    ...
    if (Object.keys) {
        test('Mustache memory leak (#1393)', function () {
            ...
            var studentsTemplate = can.mustache('<div>{{#each students}}abc{{/each}}</div>');
            var students = new can.List([
                    'first',
                    'second'
                ]);
            var frag = studentsTemplate({ students: students });
            can.append(can.$('#qunit-fixture'), frag);
...
define('can/construct/super/super_test', [
...
/*can@2.2.1#map/lazy/nested_reference_test*/
define('can/map/lazy/nested_reference_test', [
...
/*can@2.2.1#map/lazy/observe_test*/
...
/*map/attributes/attributes_test*/
...
define('can/map/define/define_test', [
    ...
    test('value generator can read other properties', function () {
        ...
        equal(n.attr('firstNumber'), 1, prefix + 'traditional can.Map style property definition');
...
/*list/sort/sort_test*/
define('can/list/sort/sort_test', [
    'can/list/sort/sort',
    'can/test/test',
    'can/view/mustache/mustache',
    'can/view/stache/stache',
...
define('can/control/plugin/plugin_test', [
...
define('can/view/modifiers/modifiers_test', [
    ...
    test('modifier with a deferred', function () {
        ...
        $('#qunit-fixture').html(can.test.path('view/test/deferred.ejs'), foo);
...
define('can/util/object/isplain/isplain_test', [
...
define('can/util/object/object_test', [
    'can/util/object/object',
    'can/util/object/isplain/isplain_test',
    'steal-qunit'
], function () {
...
define('can/view/bindings/bindings_test', [
    ...
    if (typeof document.getElementsByClassName === 'function') {
        test('can-event handlers', function () {
            ...
            template = can.view.mustache('<div>' + '{{#each foodTypes}}' + '<can-event-args-tester class=\'with-args\' can-click=\'{withArgs @event @element @viewModel @viewModel.test . title content=content}\'/>' + '{{/each}}' + '</div>');
    ...
    test('can-EVENT removed in live bindings doesn\'t unbind (#1112)', function () {
        var flag = can.compute(true), clickHandlerCount = 0;
        var frag = can.view.mustache('<div {{#if flag}}can-click=\'foo\'{{/if}}>Click</div>')({
                flag: flag,
                foo: function () {
                    clickHandlerCount++;
    ...
    test('can-value select multiple applies initial value, when options rendered from array (#1414)', function () {
        var template = can.view.mustache('<select can-value=\'colors\' multiple>' + '{{#each allColors}}<option value=\'{{value}}\'>{{label}}</option>{{/each}}' + '</select>');
        var map = new can.Map({
                colors: [
                    'red',
                    'green'
...
define('can/util/string/deparam/deparam_test', ['can/util/string/deparam/deparam'], function () {
...
define('can/util/string/string_test', [
    'can/util/string/string',
    'can/util/string/deparam/deparam_test',
    'steal-qunit'
], function () {
...
define('can/util/attr/attr_test', [
    ...
    test('template attr updating', function () {
        var template = can.mustache('<div my-attr=\'{{value}}\'></div>'), compute = can.compute('foo');
...
define('can/view/autorender/autorender_test', [
    'can/test/test',
    'steal-qunit'
], function () {
